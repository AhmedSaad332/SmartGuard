...
async def signup(request: SignupRequest):
    """Register a new user - Sync to both local database and Supabase"""
    init_db()
    conn = get_db()
    c = conn.cursor()
    ...
    try:
        ...
        try:
            sb_response = supabase.table("users").select("email, status").eq("email", request.email).execute()
            if sb_response.data and len(sb_response.data) > 0:
                sb_user = sb_response.data[0]
                sb_status = sb_user.get("status", "unknown")
                if sb_status == "approved":
                    ...
                    raise HTTPException(status_code=400, detail="This email already has a pending registration request.")
        except HTTPException:
            raise
        except Exception as supabase_error:
            print(f"❌ Failed to check Supabase for existing user: {supabase_error}")
            raise HTTPException(status_code=500, detail=f"Failed to check Supabase: {str(supabase_error)}")
        
        # Check if there's already a pending signup request in local DB
        c.execute("SELECT email FROM signup_requests WHERE email = ?", (request.email,))
        existing_request = c.fetchone()
        
        if existing_request:
            raise HTTPException(status_code=400, detail="This email already has a pending registration request.")
        
        # Insert into local signup_requests table
        request_id = secrets.token_hex(16)
        password_hash = hash_password(request.password)
        
        c.execute("""
            INSERT INTO signup_requests (id, email, full_name, password_hash, organization, role, status, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (request_id, request.email, request.full_name, password_hash, request.organization, request.role, 'pending', datetime.now()))
        
        conn.commit()
        
        print(f"Created signup request for {request.email} in local DB")
        
        # Sync to Supabase - Let Supabase auto-generate the ID (bigint)
        try:
            sb_response = supabase.table("users").insert({
                "email": request.email,
            ...
            # Get the Supabase-generated ID and update local DB with it
            if sb_response.data and len(sb_response.data) > 0:
                supabase_id = sb_response.data[0].get("id")
                # Store Supabase ID in a separate column or in the existing id column as string
                # For now, keep the local hex ID and store Supabase ID in a comment or separate field
                print(f"✅ Synced signup request to Supabase for {request.email} with Supabase ID: {supabase_id}")
            else:
                print(f"⚠️ Supabase insert succeeded but no data returned")
        except Exception as supabase_error:
            print(f"⚠️ Failed to sync to Supabase: {supabase_error}")
            print(f"⚠️ User created in local DB only. Admin will need to sync to Supabase manually.")
            # Don't fail the signup - local DB is source of truth
...
async def signin(request: SigninRequest):
    """Sign in user - Check Supabase for role and status"""
    ...
    c = conn.cursor()
    try:
        # Check Supabase for user role and status FIRST
        sb_user = None
        sb_role = None
        sb_status = None
        sb_id = None
        sb_full_name = None
        sb_organization = None
        ...
        try:
            ...
            if sb_response.data and len(sb_response.data) > 0:
                ...
                sb_role = sb_user.get("role")
                sb_status = sb_user.get("status")
                sb_full_name = sb_user.get("full_name")
                sb_organization = sb_user.get("organization")
                print(f"✅ Found user in Supabase: role={sb_role}, status={sb_status}")
        except Exception as supabase_error:
            print(f"⚠️ Failed to check Supabase: {supabase_error}")
            raise HTTPException(status_code=500, detail=f"Failed to check Supabase: {str(supabase_error)}")
        ...
        # Check local database for password verification
        print(f"Checking local database for password verification")
        c.execute("SELECT id, email, full_name, password_hash, organization, role, status FROM users WHERE email = ?",
                     (request.email,))
        user = c.fetchone()
        
        if user:
            user_id, email, full_name, password_hash, organization, role, user_status = user
            print(f"User {request.email} found in local database")
            
            print(f"Verifying password for local user")
            ...
            # Use Supabase role instead of local role
            token = generate_token(str(sb_id), email, sb_organization, sb_role)
            ...
            try:
                ...
                print(f"✅ Updated last_login in Supabase for {email}")
            except Exception as supabase_error:
                print(f"⚠️ Failed to update last_login in Supabase: {supabase_error}")
            ...
            return {
                ...
                "user": {
                    "id": str(sb_id),
                    "email": email,
                    "full_name": sb_full_name or full_name,
                    "organization": sb_organization or organization,
                    "role": sb_role,
                    "status": sb_status
                }
            }
        else:
            print(f"User {request.email} not found in local database")
            print(f"⚠️ User exists in Supabase but not in local database")
            print(f"⚠️ Creating user in local database from Supabase data")
            
            # Create user in local database from Supabase data
            # Note: We don't have the password, so we'll need to handle this
            # For now, we'll create with a placeholder and require password reset
            ...
            c.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (str(sb_id), request.email, sb_full_name or "Unknown", password_hash, sb_organization or "Unknown", sb_role or "security_man", sb_status))
            conn.commit()
            print(f"✅ Created user in local database from Supabase data")
            
            # Return error asking user to reset password
            raise HTTPException(status_code=403, detail="Account synced from Supabase. Please reset your password to login.")
    ...
    try:
        ...
        try:
            ...
            if sb_response and sb_response.data and len(sb_response.data) > 0:
                ...
                # Create user in Supabase if not found
                try:
                    # Get user data from local DB
                    c.execute("SELECT * FROM signup_requests WHERE id = ?", (request_id,))
                    local_request = c.fetchone()
                    if local_request:
                        rid, email, full_name, password_hash, organization, role, status, reason, created_at, reviewed_at, reviewed_by = local_request
                        ...
                        supabase_synced = True
                        print(f"✅ Created and approved user in Supabase: {email}")
                except Exception as create_error:
                    print(f"⚠️ Failed to create user in Supabase: {create_error}")
                    supabase_error = str(create_error)
        ...
        # Sync to Supabase - ensure user is in users table with approved status
        try:
            ...
            print(f"✅ Synced approved user to Supabase: {email}")
        except Exception as supabase_error:
            print(f"❌ Failed to sync approved user to Supabase: {supabase_error}")
            # Don't fail the request - local DB is source of truth
