...
def process_video_stream(video_source="rtsp://admin:CJNAKZ@192.168.1.5:554/h264/ch1/main/av_stream", video_id=None, organization=None):
    """
    ...
    # Use rtsp_cam1 as default video_id if not provided
    if video_id is None:
        video_id = "rtsp_cam1"
    ...
    # Initialize model with robust error handling and retry logic
    model = None
    max_retries = 3
    retry_delay = 1.0
    ...
    for attempt in range(max_retries):
        try:
            print(f" [STREAM-PROCESSOR] Model loading attempt {attempt + 1}/{max_retries}")
            model = AbnormalModel()
            
            # Check if model object was created successfully
            if model is None:
                print(f" [STREAM-PROCESSOR] Failed to create model object")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                else:
                    print(" [STREAM-PROCESSOR] Model creation failed after all retries")
                    model = None
                    break
            
            # Try to load the model
            print(f" [STREAM-PROCESSOR] Attempting to load model: {model.model_path}")
            if model.load():
                print(f" [STREAM-PROCESSOR] Model loaded successfully: {type(model.model)}")
                print(" [STREAM-PROCESSOR] Real Model Initialized - Using TensorFlow for detection")
                break
            else:
                print(f" [STREAM-PROCESSOR] Model loading failed: {model.model_path}")
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
                else:
                    print(" [STREAM-PROCESSOR] Model loading failed after all retries")
                    model = None
                    break
                    
        except Exception as e:
            print(f" [STREAM-PROCESSOR] Model initialization error: {e}")
            print(f" [STREAM-PROCESSOR] Error details: {str(e)}")
            model = None
    
    # Final model availability check
    if model is None:
        print(" [STREAM-PROCESSOR] CRITICAL: No model available - System will not perform detection")
        print(" [STREAM-PROCESSOR] Please check:")
        print("  1. TensorFlow installation")
        print("  2. Model file accessibility")
        print("  3. Model compatibility with TensorFlow version")
        print("  4. Python environment configuration")
    else:
        print(" [STREAM-PROCESSOR] Model initialization complete - Ready for detection")
        # Start baseline calibration for domain adaptation
        model.start_baseline_calibration()
    ...
    while True:
        ...
        if cap is None:
            print(f"Attempting to open RTSP stream...")
            try:
                ...
                if test_cap.isOpened():
                    ...
                    test_cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Reduce buffer size for lower latency
                    test_cap.set(cv2.CAP_PROP_FPS, 15)        # Set FPS to 15 for RTSP streams
                    ...
                    if ret and frame is not None:
                        print(f"Successfully opened RTSP stream")
                        cap = test_cap
                        use_fallback = False
                    else:
                        print(f"RTSP stream opened but no frames received")
                        test_cap.release()
                else:
        ...
        if use_fallback:
            ...
            try:
                ...
                # Force resize RTSP frames to expected resolution (640x480)
                if not use_fallback and frame.shape[1] > 1000:  # If width > 1000px (likely RTSP high-res)
                    frame = cv2.resize(frame, (640, 480), interpolation=cv2.INTER_AREA)
                    print(f" [RESOLUTION-FIX] RTSP frame resized from original to 640x480")
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".mp4") as tmp:
                ...
                try:
                    ...
                    if model is not None:
                        print(f" [DEBUG] Model loaded: {model.is_loaded()}, Running prediction...")
                        confidence = model.predict(frames_tensor)
                        is_abnormal = confidence > model.adaptive_threshold
                        print(f" [RAW-CONF] Model confidence: {confidence:.4f}, Threshold: {model.adaptive_threshold:.3f}, Result: {'ABNORMAL' if is_abnormal else 'NORMAL'}")
                        print(f" [DEBUG] Confidence > Threshold? {confidence:.4f} > {model.adaptive_threshold:.3f} = {is_abnormal}")
                    else:
                        print(" [STREAM-PROCESSOR] ⚠️ Model not available, skipping inference")
                        confidence = 0.0
                        is_abnormal = False
                    ...
                    if not is_abnormal:
                        print(f" [DETECTION] ℹ️ NORMAL behavior. Confidence: {confidence:.4f} <= Threshold: {model.adaptive_threshold:.3f}")

                    # Broadcast alert if abnormal
                    if is_abnormal:
                        print(f" [DETECTION] ✅ ABNORMAL detected! Confidence: {confidence:.4f} > Threshold: {model.adaptive_threshold:.3f}")
